...
function connecter(event) {
    ...
    document.querySelector(
        '[data-espace="' + espaceMap[roleActuel] + '"]',
    ).style.display = "block";
    document.getElementById("menu-" + espaceMap[roleActuel]).style.display =
        "block";
    document.getElementById("etiquette-role").textContent = roleActuel;
    document.getElementById("role-utilisateur").textContent =
        libelleMap[roleActuel];
...
function afficherVue(espace, idVue, bouton) {
    document
        .querySelectorAll('[data-espace="' + espace + '"] .vue')
        .forEach((v) => v.classList.remove("actif"));
